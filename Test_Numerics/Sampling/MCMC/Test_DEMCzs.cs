...
namespace Sampling.MCMC
{
    ...
    public class Test_DEMCzs
    {
        ...
        public void Test_DEMCzs_Coles_EQ6()
        {
            ...
            {
                // Get GEV parameters
                double loc = parameters[0];
                double scl = parameters[1];
                double shp = parameters[2];
        ...
        /// </summary>
        [TestMethod]
        public void Test_DEMCzs_Coles_EQ7()
        {
            ...
            var priors = new List<IUnivariateDistribution>
            {
                new Uniform(100, 300), // Change Point
                new Uniform(-1000, 1000), // Location 1
                new Uniform(0, 100), // Scale 1
                new Uniform(-10, 10), // Shape 1
                new Uniform(-1000, 1000), // Location 2
                new Uniform(0, 100), // Scale 2
                new Uniform(-10, 10), // Shape 2
            };
            ...
            double logLikelihood(IList<double> parameters)
            {
                ...
                // Data likelihood
                double w1 = Ny * k / 366d;
                double w2 = Ny * (366d - k) / 366d;
                double lh1 = 0, lh2 = 0;
                for (int i = 0; i < data.Length; i++)
                {
                    if (wyDays[i] < k)
                    {
                        lh1 += Math.Log(scl1) + (1 + 1 / shp1) * Math.Log(1 + shp1 * ((data[i] - loc1) / scl1));
                    }
                    else
                    {
                        lh2 += Math.Log(scl2) + (1 + 1 / shp2) * Math.Log(1 + shp2 * ((data[i] - loc2) / scl2));
                    }
                }
                lh1 += w1 * Math.Pow(1 + shp1 * ((u - loc1) / scl1), -1 / shp1);
                lh2 += w2 * Math.Pow(1 + shp2 * ((u - loc2) / scl2), -1 / shp2);
                double logLH = -1 * (lh1 + lh2);
